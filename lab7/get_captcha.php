...
$noiseColor = "6d87cf"; 
$character = 0;
while ($character < $totalCharacters) { //creating captcha string one by one
...
for( $captchaDotsCount=0; $captchaDotsCount<$randomDots; $captchaDotsCount++ ) { //creating dots inside of image
imagefilledellipse(
	 $captchaImage,
...
for( $captchaLinesCount=0; $captchaLinesCount<$randomLines; $captchaLinesCount++ ) { //creating lines inside of image
	imageline(
		$captchaImage,
		mt_rand(0,$captchaWidth),
		mt_rand(0,$captchaHeight),
...
$_SESSION['captcha'] = $captcha; //we are storing our captcha text in session variable
